...
class NN_Model_3_Layers(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2):
        
        super(NN_Model_3_Layers, self).__init__()
...
def applying_kfold_validation_NN(model, X_train, y_train, kfolds, epochs, learning_rate = 0.01, 
    ...
    for train_idx, val_idx in kf.split(X_train):
        ...
        Y_test  = y_train.iloc[val_idx].values

        X_train1, X_val = torch.tensor(X_train_scaled, dtype=torch.float32), torch.tensor(X_test_scaled, dtype=torch.float32)
        y_train1, y_val = torch.tensor(Y_train, dtype=torch.float32).view(-1, 1), torch.tensor(Y_test, dtype=torch.float32).view(-1, 1)
        ...
        with torch.no_grad():
            ...
            r2_scores.append(r2_score(val_outputs, y_val))
            mae_scores.append(mean_absolute_error(val_outputs, y_val))
            mse_scores.append(mean_squared_error(val_outputs, y_val))
    ...
    return val_losses, losses_per_epoch, model, criterion, scale_x 
...
def main(model, feature_df, y_val_df, kfolds, epochs):
    ...
    val_losses, losses_per_epoch, model, criterion, scale_x = applying_kfold_validation_NN(  model, 
                                                                                    X_train,
                                                                                    y_train, 
                                                                                    kfolds, 
                                                                                    epochs, 
    ...
    y_pred, test_loss = test_model(model, X_test, y_test, scale_x, criterion)
    
    return val_losses, losses_per_epoch, y_test, y_pred, test_loss
...
if __name__ == "__main__":
    ...
    # Create model
    model = NN_Model_3_Layers(input_size, hidden_size1, hidden_size2)

    # Run main function
    val_losses, losses_per_epoch, y_test, y_pred, test_loss = main(model, 
                                                                    feature_df, 
                                                                    y_val_df, kfolds, epochs)
